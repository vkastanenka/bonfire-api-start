...
func NewHandler(hub *gateway.Hub, userCache UserCache, ticketCache TicketCache, bind *httpio.Bind) *Handler {
	return &Handler{
		hub:         hub,
		userCache:   userCache,
		ticketCache: ticketCache,
...
func (h *Handler) ServeWS(w http.ResponseWriter, r *http.Request) error {
	...
	client := gateway.NewClient(ctx, userID, sessionID, conn)
