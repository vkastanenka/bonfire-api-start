...
func (s *ChannelService) CreateGroup(ctx context.Context, rawUserID uuid.UUID, rawPeerIDs []uuid.UUID) error {
	...
	// TODO: 1. Convert to piped aggregate
...
func (s *ChannelService) Get(ctx context.Context, rawUserID, rawChannelID, rawMessageID uuid.UUID) (*Channel, []MemberView, []MessageView, error) {
	...
	/*
		TODO
		Piped redis get for member get by channel ids AND channel get
		DB fallback and cache write
		Redis get messages list around by channel id
		DB fallback and cache write
		Redis pipe reactionRepo.GetBatchSummaryByMessageIDs, userRepo.GetBatch, presenceCache.GetBatch
		DB fallback and cache write
	*/
...
func (s *ChannelService) GetSidebar(ctx context.Context, rawUserID uuid.UUID) ([]SidebarView, error) {
	...
	/*
		TODO
		Redis get for memberRepo.ListVisibleByUserID
		DB fallback and cache write
		Redis pipe for repo.GetBatch and memberRepo.GetBatchByChannelIDs
		DB fallback and cache write
		Redis pipe for userRepo.GetBatch and presenceCache.GetBatch
		DB fallback and cache write
	*/
...
func (s *ChannelService) UpdateGroup(ctx context.Context, rawUserID, rawChannelID uuid.UUID, rawName, rawIconURL *string) (*Channel, error) {
	...
	/*
		TODO
		Redis get for memberRepo.Get
		DB fallback and cache write
		Redis pipe for cache.Delete and .messageCache.SetBatch
	*/
